...
/// with an [ExpansionTileImpController] that's owned by a stateful widget
/// or look up the tile's automatically created [ExpansionTileImpController]
/// with [ExpansionTileImpController.of]
///
/// The controller's [expand] and [collapse] methods cause the
/// the [ExpansionTile] to rebuild, so they may not be called from
/// a build method.
class ExpansionTileImpController {
  /// Create a controller to be used with [ExpansionTile.controller].
  ExpansionTileImpController();

  _ExpansionTileImpState? _state;
  ...
  /// Finds the [ExpansionTileImpController] for the closest [ExpansionTile] instance
  ...
  ///
  /// {@tool dartpad}
  /// Typical usage of the [ExpansionTileImpController.of] function is to call it from within the
  ...
  /// the [ExpansionTileImpController] (since it's "above" the widget
  ...
  /// can obtain the [ExpansionTileImpController]. With this approach you
  /// would have an outer widget that creates the [ExpansionTile]
  /// populated by instances of your new inner widgets, and then in
  /// these inner widgets you would use [ExpansionTileImpController.of].
  static ExpansionTileImpController of(BuildContext context) {
    final _ExpansionTileImpState? result =
        context.findAncestorStateOfType<_ExpansionTileImpState>();
    if (result != null) {
      return result._tileController;
    }
    throw FlutterError.fromParts(<DiagnosticsNode>[
      ErrorSummary(
        'ExpansionTileImpController.of() called with a context that does not contain a ExpansionTile.',
      ),
      ErrorDescription(
        'No ExpansionTile ancestor could be found starting from the context that was passed to ExpansionTileImpController.of(). '
      ...
      ErrorHint(
        ...
        'documentation for ExpansionTileImpController.of():\n'
      ...
      ErrorHint(
        ...
        'your new inner widgets, and then in these inner widgets you would use ExpansionTileImpController.of().\n'
        ...
        'using the ExpansionTileImpController.of() function.',
      ),
  ...
  /// encloses the given context and returns its [ExpansionTileImpController].
  ///
  ...
  ///    documentation.
  static ExpansionTileImpController? maybeOf(BuildContext context) {
    return context
        .findAncestorStateOfType<_ExpansionTileImpState>()
        ?._tileController;
  }
}
...
///
/// {@tool dartpad}
/// This example demonstrates how an [ExpansionTileImpController] can be used to
...
class ExpansionTileImp extends StatefulWidget {
  /// Creates a single-line [ListTile] with an expansion arrow icon that expands or collapses
  /// the tile to reveal or hide the [children]. The [initiallyExpanded] property must
  /// be non-null.
  const ExpansionTileImp({
    super.key,
    this.leading,
    required this.title,
    this.subtitle,
  ...
  /// by a widget within the tile, [ExpansionTileImpController.of] may be more convenient
  /// than supplying a controller.
  final ExpansionTileImpController? controller;
  ...
  /// through an [ExpansionTileImpController].
  final bool enabled;
  ...
  @override
  State<ExpansionTileImp> createState() => _ExpansionTileImpState();
}

class _ExpansionTileImpState extends State<ExpansionTileImp>
  ...
  late ExpansionTileImpController _tileController;
  ...
  void initState() {
    ...
    _tileController = widget.controller ?? ExpansionTileImpController();
  ...
  @override
  void didUpdateWidget(covariant ExpansionTileImp oldWidget) {
